...
description: "Cria ou atualiza endpoints ASP.NET Core com Swagger completo no Leadfy. Use para novas rotas, ajustes de verbo HTTP, ActionResult, autenticacao, XML comments, ProducesResponseType e documentacao OpenAPI em controllers do backend."
...
4. Mapeie os status codes realmente possiveis a partir de `return`, `catch` e excecoes esperadas.
5. Confirme se a mudanca exige atualizacao de contrato em App ou DTO, mesmo que a edicao principal esteja no controller.
...
Confirme rota, verbo, parametros de rota ou query, request body, formato da resposta e necessidade de autenticacao.
