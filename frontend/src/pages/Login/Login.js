    ...
    <main className={style.page}>
      <section className={style.brandPanel}>
        ...
        <div className={style.brandContent}>
          ...
          <h1 className={style.title}>Acesse seu painel com um fluxo simples e seguro.</h1>
          <p className={style.description}>
            Entre com sua conta para acompanhar leads, oportunidades, produtos e a operação comercial em um só lugar.
          </p>

          <div className={style.metrics}>
            <div className={style.metricCard}>
              <strong>JWT ativo</strong>
              <span>Sessão persistida no navegador para acesso contínuo.</span>
            </div>
            <div className={style.metricCard}>
              <strong>Operação centralizada</strong>
              <span>Visual vermelho, contraste forte e foco rápido nas ações do dia.</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className={style.formPanel}>
        <div className={style.formCard}>
          ...
          <form className={style.form} onSubmit={(event) => {
            ...
            {submitError && <div className={style.errorBanner}>{submitError}</div>}
            ...
            <div className={style.field}>
              ...
              <input
                ...
                {...register('email')}
              />
              {errors.email && <span className={style.fieldError}>{errors.email.message}</span>}
            </div>
            ...
            <div className={style.field}>
              ...
              <input
                ...
                {...register('password')}
              />
              {errors.password && <span className={style.fieldError}>{errors.password.message}</span>}
            </div>
